...
%             C(i,j,k) = simplify(diff(M(k,j), q(i))) + ...
...
syms w_n epsilon N Ks Jm Kt Ke Ra Ds Dm real
Ar = [     0                1;
      -w_n^2   -2*epsilon*w_n];
  
A = [            0                                   1;
     ((N^(-2))*Ks)/Jm   ((N^(-2))*Ds-Dm-(Kt*Ke/Ra))/Jm];
...
simplify(inv((B')*B)*(B')*Br) % M
simplify(inv((B')*B)*(B')*(A-Ar)) % L
